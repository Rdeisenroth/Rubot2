...
const command: ButtonInteraction = {
    ...
    execute: async (client, interaction) => {
        ...
        // Leave the Queue
        let entry = queue.entries.splice(queue.entries.findIndex(x => x.discord_id === interaction.user.id), 1)[0];
        await g.save();
        try {
            let member = client.guilds.cache.get(g._id)?.members.cache.get(interaction.user.id);
